...
#[derive(
    BorshDeserialize, BorshSerialize, Copy, Clone, PartialOrd, Ord, PartialEq, Eq, Debug, Hash,
)]
pub struct Sudoku(pub(crate) SudokuArray);

impl Sudoku {
    pub fn from_two_dimensional_array(array: &SudokuTwoDimensionalArray) -> Sudoku {
        let mut sudoku = Sudoku([0; 81]);

        for x in 0..9 {
            for y in 0..9 {
                sudoku.0[x * 9 + y] = array[x][y];
            }
        }
    ...
    pub fn to_two_dimensional_array(&self) -> SudokuTwoDimensionalArray {
        let mut multi_array: [[u8; 9]; 9] = [[0; 9]; 9];

        for x in 0..9 {
            for y in 0..9 {
                multi_array[x][y] = self.0[x * 9 + y];
            }
        }
...
impl Sudoku {
    ...
    pub fn generate_solved(rng: &mut StdRng) -> Self {
    ...
    pub fn generate(rng: &mut StdRng) -> Self {
        Sudoku::generate_with_symmetry(Symmetry::HalfRotation, rng)
    }
    ...
    /// whilst also upholding the symmetry.
    /// Most puzzles generated by this are easy.
    pub fn generate_with_symmetry(symmetry: Symmetry, rng: &mut StdRng) -> Self {
        Sudoku::generate_with_symmetry_from(Sudoku::generate_solved(rng), symmetry, rng)
    }
    ...
    pub fn generate_from(sudoku: Sudoku, rng: &mut StdRng) -> Self {
        Sudoku::generate_with_symmetry_from(sudoku, Symmetry::None, rng)
    }
    ...
    pub fn generate_with_symmetry_from(
        mut sudoku: Sudoku,
        symmetry: Symmetry,
        rng: &mut StdRng,
    ) -> Self {
    ...
    pub fn shuffle(&mut self, rng: &mut StdRng) {
        let transformation = crate::board::canonicalization::Transformation::random(rng);
...
impl fmt::Display for SudokuBlock {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        for (digit, cell) in self
            .0
            .iter()
            .cloned()
            .map(Digit::new_checked)
            .zip(Cell::all())
        {
            #[rustfmt::skip]
...
mod test {
    ...
    fn test_symmetry_composite_symmetries() {
        ...
        fn multi_symmmetry_corresponding_cells(
            symmetries: &[Symmetry],
            cell: usize,
        ) -> HashSet<usize> {
